-- Wojak: the ground truth.
-- A wojak is a seed plus coordinates plus accreted state.
-- Axes are not declared; they emerge from coordinates wojaks take.
-- Pepos and memos are not stored here; they are queries over the field.
module Wojak
  ( Coord (..), CoordType (..)
...
data Coord
  ...
  | Matrix   [[Coord]]
  | Tensor   [Int] [Coord]
  | Graph    [Coord] [(Int, Int, Coord)]
  | Lattice  [Coord] [(Int, Int)]
  | Manifold [([Float], Coord)]
 deriving (Eq, Show)
...
edgeDist e1 e2 =
 ...
 where
  gap (i, j, w) = case weightsAt i j e2 of
    []       -> coordNorm w
    (w2 : _) -> coordDist w w2
  weightsAt i j es = [w | (x, y, w) <- es, sameEnds i j x y]
...
chartDist p1 p2 =
 ...
 where
  nearest (xs, v) pts =
    minOrBig [l2 (zipWith (-) xs ys) + coordDist v w | (ys, w) <- pts]
...
spectralDist a b = l2 (zipPad (leaves a) (leaves b))
 where
  zipPad xs ys =
    zipWith (-) xs ys ++ drop n xs ++ drop n ys
   where n = min (length xs) (length ys)
...
scaleCoord k c = case c of
  ...
  Manifold pts  -> Manifold [(map (k *) p, scaleCoord k v) | (p, v) <- pts]
...
coordToCbor c = case c of
  ...
  Matrix rows  -> CList [CText "matrix",
                         CList [CList (map coordToCbor r) | r <- rows]]
  ...
  Lattice xs o -> CList [CText "lattice",
                         CList (map coordToCbor xs),
                         CList [CList [CInt i, CInt j] | (i, j) <- o]]
  Manifold pts -> CList [CText "manifold",
                         CList [CList [CList (map CFloat p), coordToCbor v]
                               | (p, v) <- pts]]
...
coordFromCbor v = case v of
  ...
  CList [CText "vector", CList xs] -> Vector <$> mapM coordFromCbor xs
  CList [CText "ray", o, d] ->
...
intFrom x = case x of { CInt n -> Just n; _ -> Nothing }
...
data Wojak = Wojak
  { wjSeed   :: Seed
  , wjCoords :: [(String, Coord)]
  , wjState  :: [(String, CborValue)]
  }
 deriving (Eq, Show)
...
-- ---- CBOR for wojaks and connections -------------------------

wojakToCbor :: Wojak -> CborValue
wojakToCbor w = cmap
  [ ("seed", wjSeed w)
  , ("coords", CMap [(CText ax, coordToCbor c) | (ax, c) <- wjCoords w])
  , ("state", CMap [(CText k, v) | (k, v) <- wjState w])
  ]

wojakFromCbor :: CborValue -> Maybe Wojak
wojakFromCbor v =
  case (cborLookup "seed" v, cborLookup "coords" v, cborLookup "state" v) of
...
connToCbor c = cmap [("a", cnA c), ("b", cnB c), ("ctx", cnCtx c)]

connFromCbor :: CborValue -> Maybe Connection
connFromCbor v =
  case (cborLookup "a" v, cborLookup "b" v, cborLookup "ctx" v) of
